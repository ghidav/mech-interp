...
def plot_pc(activations, prompts, rows, cols, n_comp=2):
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            if n_comp == 1:
                fig.add_trace(go.Scatter(x=x_values, y=kde_0(x_values), mode='lines', name='Class 0', line=dict(color='red')), row=i+1, col=j+1)
                fig.add_trace(go.Scatter(x=x_values, y=kde_1(x_values), mode='lines', name='Class 1', line=dict(color='blues')), row=i+1, col=j+1)
            else:
...
def plot_single_probes(scores_df, rows, cols):
    n_layers = rows*cols
    fig = make_subplots(rows=rows, cols=cols, subplot_titles=[f'L {i+1}' for i in range(n_layers)], horizontal_spacing=0.05, vertical_spacing=0.05)
...
def plot_multi_probes(scores_dfs, rows, cols):
    n_layers = rows*cols
    fig = make_subplots(rows=rows, cols=cols, subplot_titles=[f'L {i+1}' for i in range(n_layers)], horizontal_spacing=0.05, vertical_spacing=0.05, shared_yaxes=True)
